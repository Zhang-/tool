...
define('LOG_PATH','/data0/log-data/');

define('DISPLAY_REVERSE',true); // true = displays log entries starting with the most recent
//define('DIRECTORY_SEPARATOR','/');

$GLOBALS['fileAllowtype'] = ['.log', '.err', '.text'];

function checkAllowFile($fileName)
{

	$cutStr = '.';
    $lastLen  = strrpos($fileName, $cutStr);
    $fileType = substr($fileName, $lastLen);
    return in_array($fileType, $GLOBALS['fileAllowtype']);
}
...
function get_log_files($dir, &$results = array()) {
	...
	foreach($files as $key => $value){
        ...
        !isset($files_list) && $files_list = [];
	foreach ($files_list as $path)
	{
		if (checkAllowFile($path))
		{
...
?>
 ...
 <form action="" method="get">
	select count to display : <input type="hidden" name="p" value="<?php echo $log ?>">
...
<footer>
<p class="credits"><a href="//mazhaer.com">Script by Jason_Hooper</a>, thanks to <a href="//mazhaer.com/">mazhaer.com</a></p>
</footer>
</div>
    </div>
</div>
